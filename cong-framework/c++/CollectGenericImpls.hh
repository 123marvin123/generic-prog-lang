...
namespace cong::lang::intern
{
    ...
    auto get_impl_value_forwarded(ActualExpArg&& e, ActualTupleOfExpArg&& t) {
        return Impl_::template Call<Exp_, TupleOfExp_>::ApplyTime::template Call<Exp_, TupleOfExp_>::call(
            std::forward<ActualExpArg>(e),
            std::forward<ActualTupleOfExpArg>(t)
        ).native();
    }

    template<class Exp_, class TupleOfExp_, class TupleOfImpl_, typename FwdExpArg, typename FwdTupleOfExpArg, size_t... Is>
    auto find_best_impl_runtime(FwdExpArg&& exp_arg, FwdTupleOfExpArg&& tuple_exp_arg, std::index_sequence<Is...>) {
        ...
        auto evaluate_and_update = [&](auto idx_constant) {
            ...
            double current_val = get_impl_value_forwarded<CurrentImpl, Exp_, TupleOfExp_>(
        ...
        if (best_idx == static_cast<size_t>(-1) && std::tuple_size_v<TupleOfImpl_> > 0) {
            throw std::runtime_error("Konnte die beste Implementierung nicht bestimmen (alle Werte waren max_double oder es gab ein anderes Problem).");
        }
    ...
    private:
        ...
        struct CompareImpls {
        private:
            using ATime_ = typename ImplA_::template Call<Exp_, TupleOfExp_>::ApplyTime::template Call<Exp_, TupleOfExp_>::Type;    
            using BTime_ = typename ImplB_::template Call<Exp_, TupleOfExp_>::ApplyTime::template Call<Exp_, TupleOfExp_>::Type;
        
            static_assert(core::Truthy::Call<typename IsStatic<typename ImplA_::template Call<Exp_, TupleOfExp_>::Type>::Type>::call(), "Impl_ must return a static result");
            static_assert(core::Truthy::Call<typename IsStatic<ATime_>::Type>::call(), "Impl_ must return a static time complexity");
    
            static_assert(core::Truthy::Call<typename IsStatic<typename ImplB_::template Call<Exp_, TupleOfExp_>::Type>::Type>::call(), "Other_ must return a static result");
            static_assert(core::Truthy::Call<typename IsStatic<BTime_>::Type>::call(), "Other_ must return a static time complexity");
        public:
    ...
    private:
        ...
        private:
            using NextTuple_ = std::conditional_t<
                core::Truthy::Call<typename IsStatic<typename Impl_::template Call<Exp_, TupleOfExp_>::Type>::Type>::call(), 
    ...
    private:
        ...
        private:
            ...
            using Remaining_ = std::conditional_t<
                core::Falsy::Call<typename IsStatic<typename Impl_::template Call<Exp_, TupleOfExp_>::Type>::Type>::call(),
